<div {{ $attributes->merge(['class' => '
   prose dark:prose-invert max-w-none
   [&_h1]:text-3xl [&_h1]:font-bold [&_h1]:mb-6
   [&_h2]:text-2xl [&_h2]:font-bold [&_h2]:mb-4
   [&_h3]:text-xl [&_h3]:font-bold [&_h3]:mb-3
   [&_p]:mb-4 [&_p]:leading-relaxed [&_p]:text-zinc-600 dark:[&_p]:text-zinc-300
   [&_ul]:list-disc [&_ul]:ml-6 [&_ul]:mb-4
   [&_ol]:list-decimal [&_ol]:ml-6 [&_ol]:mb-4
   [&_li]:mb-2 [&_li]:text-zinc-600 dark:[&_li]:text-zinc-300
   [&_blockquote]:border-l-4 [&_blockquote]:border-zinc-300 [&_blockquote]:pl-4 [&_blockquote]:italic
   [&_pre]:p-6 [&_pre]:my-4 [&_pre]:rounded-xl [&_pre]:bg-white dark:[&_pre]:bg-white/10 [&_pre]:border [&_pre]:border-zinc-200 dark:[&_pre]:border-white/10
   [&_code]:font-mono [&_code]:text-sm
   [&_a]:inline [&_a]:font-medium [&_a]:underline [&_a]:underline-offset-[6px]
   [&_a]:text-zinc-800 dark:[&_a]:text-white [&_a]:decoration-zinc-800/20 dark:[&_a]:decoration-white/20
   hover:[&_a]:decoration-current
   [&_strong]:font-bold [&_strong]:text-zinc-900 dark:[&_strong]:text-white
   [&_em]:italic
   [&_hr]:my-8 [&_hr]:border-zinc-200 dark:[&_hr]:border-zinc-700']) }}>
    {!! str_replace('&nbsp;', ' ', $content) !!}
</div>
